...
var SearchConditionSelectorView = /** @class */ (function () {
    function SearchConditionSelectorView(app, difficulties, abilities) {
        ...
        this.abilityChoices = this.generateChoices(this.abilityChoicesElement, this.abilities, { maxItemCount: maxNubmerOfPlayer, needDuplicatedSelect: true, needMultipleSelect: true, placeholderValue: "Select at most " + maxNubmerOfPlayer + " abilities of players to search",
    ...
    SearchConditionSelectorView.prototype.whenDecideCondition = function () {
        var abilitySelectedValue = this.abilityChoices.getValue(true);
        var abilitySelected = (Array.isArray(abilitySelectedValue)) ? abilitySelectedValue : [abilitySelectedValue];
        var targetSelectedValue = this.targetChoices.getValue(true);
        var targetSelected = (Array.isArray(targetSelectedValue)) ? targetSelectedValue : [targetSelectedValue];
        if (!StateAdminister_1.StateAdministrator.checkGameSystemEnvIsSet(this.app.state.gameSystemEnvDisplayed))
            throw new Error("閲覧する記録のゲームタイトルとゲームモードが設定されていません。");
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        selectedTargetItem = this.difficulties.find(function (ele) { return ele.id === _this.difficultySelectedID; });
    ...
    SearchConditionSelectorView.prototype.generateChoices = function (insertedElement, data, _a) {
        ...
        var result = new choices_js_1.default(insertedElement, {
            choices: data.map(function (ele) { return { value: ele.id, label: aboutLang_1.selectAppropriateName(ele, _this.app.state.language) }; }),
            placeholderValue: placeholderValue,
        ...
        if (!needDuplicatedSelect)
            return result;
        insertedElement.addEventListener("addItem", function (event) {
            result.setChoices([{ label: event.detail.label, value: event.detail.value }]);
        });
        insertedElement.addEventListener("removeItem", function (event) {
            result.clearStore();
            result.setChoices(data.map(function (ele) { return { value: ele.id, label: aboutLang_1.selectAppropriateName(ele, _this.app.state.language) }; }));
        });
        return result;
    };
