...
const contents={
    ...
    ruleClass:{
        ...
        English: "Applied Rule Class<br>(Scope of this rule class.)"
    },
    annotated:{
        Japanese:"注釈あり",
        English: "Annotated"
...
export class RuleIndexPart {
    ...
    private generateRuleIndexHTML(ruleObj: RuleAttributeAndAppliedClassInfo, language: LanguageInApplication) {
        return `<div class="__item">
                    <p><i class="${ruleObj.rule.iconCSS}"></i> ${ruleObj.rule.title}</p> <div class="__classItems">${this.generateClassDescriptionInRuleIndex(ruleObj.appliedClass, language)}</div>
                </div>`;
    }
    private generateClassDescriptionInRuleIndex(appliedClass: AppliedRuleClassResolved[], language: LanguageInApplication) {
        ...
        <div>
            ${this.generateCSSIcons(ruleClass)} (${ruleClass.scope || "--"}`
