...
export class EditorAbilityAttributePart implements EditorPart<string[]> {
    ...
    get requiredTypeInString(){
        return EditorAbilityAttributePart._requiredTypeInString
    }
    constructor({
        ...
        this.container.appendChild(this.htmlCon.elementWithoutEscaping`
            <h3 class="u-noUnderline">${generateIcooonHTML({icooonName:icooon})}${title}</h3>
        `);
