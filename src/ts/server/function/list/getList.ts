...
async function getList_UseId<T extends IItemOfResolveTableToName>(input:IReceivedDataAtServer_getlist_UseId, searchFunc:()=>Promise<T[]>){
    ...
    if ( input.id === undefined ) return result;
    return searchBasedOnIDList(input.id,result)
}
async function getList_UseSIdId<T extends IItemOfResolveTableToName>(input:IReceivedDataAtServer_getlist_UseSIdId, searchFunc:(gameSystemID:string)=>Promise<T[]>){
    if (input.start === undefined) input.start = 0;
    const end = (input.limit === undefined) ? undefined : input.start + input.limit
...
async function getList_UseSIdMIdId<T extends IItemOfResolveTableToName>(input:IReceivedDataAtServer_getlist_UseSIdMIdId, searchFunc:(gameSystemID:string,gameModeID:string)=>Promise<T[]>){
    if (input.start === undefined) input.start = 0;
    const end = (input.limit === undefined) ? undefined : input.start + input.limit
...
export const gameModeIDs    = async (database:InterfaceOfRecordDatabase,input:IReceivedDataAtServer_getlist_UseSIdId) =>  getList_UseSIdId(input,(s) => database.getGameModeCollection(s))
